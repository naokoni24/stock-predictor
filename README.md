...
- **daily-signals**: 毎日10:37 JSTに実行。株価・テクニカル指標・シグナルを更新し、5営業日後に確定したAI買い候補の本番実績を保存してから、ニュース更新・データ保持期間ルールを適用する。終値が欠損・未更新の銘柄だけを対象にした修復実行が12:37 JSTにも走る。スケジュール実行は2本とも修復モード(`REPAIR_MISSING_CLOSES_ONLY=1`)で起動し、直近の市場日(土日祝・取引終了前は直前の取引日)のシグナルが未保存の場合だけスクリプト側で通常モード(全銘柄処理)へ切り替わる。これにより土日祝や2本目の実行で同じ市場日を再処理しない(2026-09-26修正。以前は2本目の判定が旧cronのまま残り毎回通常モードで再処理しており、同じ市場日のAI買い候補が上限20件を超えていた)。実績評価・ニュース取得・保持期間処理は本実行スケジュール、または通常モードで処理した実行で行う。東証の取引終了(15:00 JST)より前の時刻だが、`scripts/fetch_and_signal.py`側の安全策(`MARKET_CLOSE_HOUR_JST`、取引終了前は当日分を除外)があるため実害はない。2026-09-04〜06の実測でGitHub Actionsのscheduleイベント自体が4〜6時間規模で恒常的に遅延することが判明したため、その遅延を見込んで15:37/17:12から11:37/13:37へ大きく前倒しした(2026-09-07調整)。2026-09-26にユーザー依頼でさらに1時間早め、10:37/12:37とした。
...
- `vercel.json`で1日2回(17:52 JST頃・19:07 JST頃)、`src/app/api/cron/repair-check`を呼び出す。12:37修復実行の想定遅延(GitHub Actions scheduleイベントは4〜6時間規模で遅延しうる)を見込んだ検知タイミングで、19:07は17:52のVercel Cron自体が飛んだ場合の最終保険(判定ロジックが冪等なので2本立てても正規の実行と競合しない)。
- このAPIは`daily-signals`の本日(JST)の取引終了(15:00)以降に開始した実行履歴をGitHub REST APIで確認し、
  実行中・完了済みの実行が1件も無い場合だけ、`repair_only=1`で`workflow_dispatch`を起動する
  (当日のシグナルが未保存ならスクリプト側で通常モードへ切り替わる)。取引終了前に開始した実行は
  前営業日までしか処理しないため数えない(2026-09-26修正)。
- 必要なVercel環境変数:

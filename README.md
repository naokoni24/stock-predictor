...
- `vercel.json`で1日2回(17:52 JST頃・19:07 JST頃)、`src/app/api/cron/repair-check`を呼び出す。13:37修復実行の想定遅延(GitHub Actions scheduleイベントは4〜6時間規模で遅延しうる)を見込んだ検知タイミングで、19:07は17:52のVercel Cron自体が飛んだ場合の最終保険(判定ロジックが冪等なので2本立てても正規の実行と競合しない)。
